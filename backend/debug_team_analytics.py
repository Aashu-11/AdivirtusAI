...
def debug_team_analytics():
    ...
    # Check if there are multiple employees but they're being incorrectly grouped
    print(f"\n🧐 Department grouping check:")
    product_employees = [emp for emp in employees_data if emp.get('department') == 'Product']
    print(f"Employees actually in Product department: {len(product_employees)}")
    
    for emp in product_employees:
        print(f"  - {emp.get('email')} | {emp.get('department')} | Competency: {emp.get('avg_competency')}")
