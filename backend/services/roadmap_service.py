...
class LearningRoadmapCrew:
    def __init__(self, user_id: str, user_skill_gaps: dict, user_learning_profile: dict):
        self.user_id = user_id
        self.user_skill_gaps = user_skill_gaps
    ...
    def _process_crew_results(self, crew_result):
        skill_gap_summary = "No summary available."
        learning_profile_summary = "No summary available."

        # Attempt to extract summaries from crew_result
        if isinstance(crew_result, dict) and 'tasks_outputs' in crew_result and isinstance(crew_result['tasks_outputs'], list):
            for task_output in crew_result['tasks_outputs']:
                if isinstance(task_output, dict):
                    desc = task_output.get('description', '').lower()
                    output = task_output.get('output', '')
                    if not output: # If output is empty or None, use a default
                        output = "Summary not generated by agent."

                    # Match based on known parts of task descriptions
                    if 'extract and analyze skill gaps' in desc:
                        skill_gap_summary = output
                    elif 'extract and interpret learning profile' in desc:
                        learning_profile_summary = output

        return {
            "title": "Personalized Learning Roadmap",
            "description": "AI-generated personalized learning roadmap based on skill gaps and learning profile",
            "skill_gaps": self.user_skill_gaps,
            "learning_profile": self.user_learning_profile,
...
def generate_user_roadmap(user_id: str) -> dict:
    ...
    try:
        supabase_tool = SupabaseDataTool()
        initial_data = supabase_tool._run(user_id=user_id)

        if "error" in initial_data:
            # Handle error case - perhaps return an error structure immediately from generate_user_roadmap
            # For now, let's ensure they are dictionaries with an error message for consistency
            user_skill_gaps = {"error": f"Failed to fetch skill_gaps: {initial_data['error']}"}
            user_learning_profile = {"error": f"Failed to fetch learning_profile: {initial_data['error']}"}
        else:
            user_skill_gaps = initial_data.get("skill_gaps")
            user_learning_profile = initial_data.get("learning_profile")

        # Ensure they are not None, provide default messages if so (as per SupabaseDataTool's own logic)
        if user_skill_gaps is None:
            user_skill_gaps = {'message': 'No baseline skill matrix found. Please complete your assessment.'}
        if user_learning_profile is None:
            user_learning_profile = {'message': 'No learning profile found. Please complete your learning style assessment.'}

        roadmap_crew = LearningRoadmapCrew(user_id, user_skill_gaps=user_skill_gaps, user_learning_profile=user_learning_profile)
