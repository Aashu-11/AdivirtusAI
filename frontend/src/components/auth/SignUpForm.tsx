...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // Check specifically for account already exists error
        if (error.message.includes('User already registered') || 
            error.message.includes('already in use') || 
            error.message.toLowerCase().includes('already exists')) {
          throw new Error('An account with this email already exists. Please use the sign in form instead.')
        }
        throw error
      }

      if (data?.user?.identities?.length === 0) {
        // This is another way to detect if the email is already in use
        setError('An account with this email already exists. Please use the sign in form instead.')
        setLoading(false)
        return
      }
              ...
              <div className="mt-6 space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl font-medium text-center
                    border border-gray-800/50 hover:bg-white/5 transition-all duration-200
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#020202] to-[#050505] text-white p-4 sm:p-6 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        <div className="bg-[#0A0A0A]/60 backdrop-blur-xl rounded-2xl p-6 sm:p-8 border border-gray-800/50 shadow-2xl relative">
          ...
          {/* Form content - same as before but with all input fields */}
          <div className="mb-8 relative">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                ...
                <p className="text-gray-400 mt-2">Embark on your personalized AI learning experience</p>
              </div>
            </div>
          </div>
